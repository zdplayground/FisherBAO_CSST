# Copy it from /home/zjding/csst_bao/fisher_pkmu/BAO_part/Fisher_sigma_alphas/numerical_method/default/, modify it for Roman BAO forecast. Only difference is the nz input file. --08-18-2023
#
import os, sys
import logging
import numpy as np
...
def main():
    ...
    parser.add_argument("--input_nzfile", type=str, help="Input file including nz, bias, f, etc.")
    ...
    parameter_file = Path("/home/zjding/csst_bao/fisher_pkmu/", "input_params.yaml")
    yaml = YAML() 
    ...
    # load the spec-z n(z), bias and growth rate
    ifile = Path(input_dir, input_nzfile)
    zmin_array, zmax_array, n_specz_array, bias_array, f_array, Sigma_z_array, Sigma_xy_array = np.loadtxt(ifile, unpack=True)
    ...
    for z_low, z_up, n_specz, f_growthrate, bias, Sigma_para, Sigma_perp in zip(zmin_array, zmax_array, n_specz_array, f_array, bias_array, Sigma_z_array, Sigma_xy_array):
        ...
        sn_photoz = 1.0       # put a fake number
        sn_specz = 1./n_specz
