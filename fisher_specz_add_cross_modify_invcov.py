...
## !!! Pa and Pb should include shot noise !!!
def cal_Fmn(kobs, mu_obs, V_survey, Pa, Pab, Pb, dPa_dm, dPab_dm, dPa_dn, dPab_dn, cross_coeff_cut):
    ...
    for i in range(len(kobs)):
        ...
        for j in range(len(mu_obs)):
            ...
            if mu_obs[j] < 0.2:
                cov_coeff = cross_cov[0, 1]/(cross_cov[0,0] * cross_cov[1,1])**0.5
                ## test the influence from the high cross correlation
                U, s, Vh = linalg.svd(cross_cov)
                s_inv = 1./s
                s_abs = np.abs(s)
                #if (s_abs[1]/s_abs[0] < 1.e-4):
                if abs(cov_coeff) > cross_coeff_cut:
                    s_inv[1] = 0.

                inv_cov = np.dot(np.dot(Vh.T, np.diag(s_inv)), U.T)
            else:
        ...
        Fmn_k[i] = integrate.trapz(temp, mu_obs) * kobs[i]**2.0
    
    Fmn = V_survey * integrate.trapz(Fmn_k, kobs)/(2*np.pi)**2.0
...
def main():
    ...
    parser.add_argument("--cross_coeff_cut", type=float, help="The upper limit for the multitracers could be correlated.", default=0.95)
    ...
    cross_coeff_cut = args.cross_coeff_cut
    ...
    for z_low, z_up in zip(zlow_bins, zup_bins):
        ...
        for i in range(num_params):
            ...
            for j in range(i, num_params):
                ...
                temp_matrix[i, j] = cal_Fmn(kcenter, mu, V_survey, Pkmu_specz_obs, Pkmu_cross, Pkmu_photoz_obs, dspecz_dpi, dcross_dpi, dspecz_dpj, dcross_dpj, cross_coeff_cut)
